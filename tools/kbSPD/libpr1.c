...
#endif
{
	...
	case OUTPUT_SDF:
		/* kbToyTracer's view plane has half-width 1 at vpdist (see
		   kbTrace.cpp), so the requested field of view maps to
		   vpdist = 1/tan(fov/2).  Resolution comes from kbtoytracer.cfg,
		   so SPD's suggestion is only recorded as a comment. */
		fprintf(gOutfile, "# SPD suggested resolution %dx%d (fov %g deg)\n",
			resx, resy, fov_angle);
		fprintf(gOutfile, "eye (%g, %g, %g)\n", from[X], from[Y], from[Z]);
		fprintf(gOutfile, "lookat (%g, %g, %g)\n", at[X], at[Y], at[Z]);
		fprintf(gOutfile, "up (%g, %g, %g)\n", up[X], up[Y], up[Z]);
		fprintf(gOutfile, "vpdist %g\n\n", 1.0 / tan(PI * fov_angle / 360.0));
		break;
 ...
 */
...
#endif
 {
	 ...
	 case OUTPUT_SDF:
		 /* Lights are scene objects in kbToyTracer: a point primitive
		    with an emission makes a point light. */
		 lib_sdf_scene_begin();
		 fprintf(gOutfile, "\tpoint (%g, %g, %g)\n", vec[X], vec[Y], vec[Z]);
		 fprintf(gOutfile, "\temission [1.0, 1.0, 1.0]\n\n");
		 break;
 ...
 */
...
#endif
 {
	 ...
	 case OUTPUT_SDF:
		 fprintf(gOutfile, "bgcolor [%g, %g, %g]\n", color[X], color[Y], color[Z]);
		 fprintf(gOutfile, "amblight [0.2, 0.2, 0.2]\n\n");
		 break;
...
#endif
{
	...
	case OUTPUT_SDF:
		/* Each surface opens its own nested aggregate: the reader applies
		   material lines to the aggregate default AND retroactively to the
		   most recent object, so a bare material block would repaint the
		   last primitive of the previous surface.  (The 2005 port nested
		   "begin List" for the same reason; BVH keeps big groups fast.)
		   Highlights and reflection are separate knobs here: ks_spec
		   drives the Phong highlight (as in the POV output), ks the
		   reflectivity; NFF-style materials that only set ks fall back
		   to it for both. */
		lib_sdf_scene_begin();
		if (gSDF_group_open)
			fprintf(gOutfile, "\tend\n\n");
		fprintf(gOutfile, "\tbegin BVH\n");
		gSDF_group_open = 1;
		fprintf(gOutfile, "\tdiffuse [%g, %g, %g]\n",
			kd * color[X], kd * color[Y], kd * color[Z]);
		fprintf(gOutfile, "\tspecular [%g, %g, %g]\n",
			ks_spec > 0 ? ks_spec : ks,
			ks_spec > 0 ? ks_spec : ks,
			ks_spec > 0 ? ks_spec : ks);
		fprintf(gOutfile, "\treflectivity %g\n", ks);
		fprintf(gOutfile, "\trefractivity %g\n", kt);
		fprintf(gOutfile, "\tindex_refract %g\n", i_of_r > 0 ? i_of_r : 1.0);
		fprintf(gOutfile, "\tPhong_exp %g\n\n", phong_pow);
		break;
