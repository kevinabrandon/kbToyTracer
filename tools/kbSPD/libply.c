			...
			case OUTPUT_SDF:
				...
				if (norm != NULL) {
					fprintf(gOutfile,
						...
						out_verts[t][0][X], out_verts[t][0][Y], out_verts[t][0][Z],
						out_verts[t][1][X], out_verts[t][1][Y], out_verts[t][1][Z],
						out_verts[t][2][X], out_verts[t][2][Y], out_verts[t][2][Z],
						out_norms[t][0][X], out_norms[t][0][Y], out_norms[t][0][Z],
						out_norms[t][1][X], out_norms[t][1][Y], out_norms[t][1][Z],
						out_norms[t][2][X], out_norms[t][2][Y], out_norms[t][2][Z]);
				} else {
					fprintf(gOutfile,
...
#endif
 {
	 ...
	 if (gRT_out_format == OUTPUT_DELAYED) {
		 ...
		 case OUTPUT_SDF:
			 ...
			 if (tot_vert == 4) {
				 fprintf(gOutfile,
					 "\tquad (%g, %g, %g) (%g, %g, %g) (%g, %g, %g) (%g, %g, %g)\n",
					 vert[0][X], vert[0][Y], vert[0][Z],
					 vert[1][X], vert[1][Y], vert[1][Z],
					 vert[2][X], vert[2][Y], vert[2][Z],
					 vert[3][X], vert[3][Y], vert[3][Z]);
			 } else {
				 /* Triangles directly; anything bigger as a triangle fan
				    (SPD polygons are convex). */
				 for (num_vert=2;num_vert<tot_vert;++num_vert)
					 fprintf(gOutfile,
						 "\ttriangle (%g, %g, %g) (%g, %g, %g) (%g, %g, %g)\n",
						 vert[0][X], vert[0][Y], vert[0][Z],
						 vert[num_vert-1][X], vert[num_vert-1][Y], vert[num_vert-1][Z],
