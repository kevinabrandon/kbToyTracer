...
char *argv[];
{
    ...
    /* -f = floorless: skip the checkerboard, for use as an instancing
       prototype.  Strip it before the stock option parser sees it. */
    for ( i = 1 ; i < argc ; i++ ) {
		if ( !strcmp( argv[i], "-f" ) ) {
			no_floor = 1;
			for ( j = i ; j < argc-1 ; j++ )
    ...
    if ( lib_gen_get_opts( argc, argv,
		...
		fprintf(stderr, "-f - omit the checkerboard floor\n");
		return EXIT_FAIL;
    }
