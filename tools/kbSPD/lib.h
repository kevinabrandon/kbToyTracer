...
/* Map the MSVC-only secure CRT calls (added upstream for VS2017) back to
   the portable equivalents on everything else. */
#if !defined(_MSC_VER)
#include <stdio.h>
#define sprintf_s(buf, size, ...)   snprintf((buf), (size), __VA_ARGS__)
#define sscanf_s                    sscanf
#define fopen_s(pfp, fname, mode)   ((*(pfp) = fopen((fname), (mode))) == NULL)
#endif
...
#define OUTPUT_SDF       20 /* kbToyTracer Scene Description File           */
#define OUTPUT_DELAYED   21 /* Needed for RTRACE/PLG output.
...
#define OUTPUT_RT_DEFAULT   OUTPUT_SDF
...
extern int  gSDF_scene_open;   /* nonzero once the SDF "begin BVH" was emitted */
extern int  gSDF_group_open;   /* nonzero while a per-surface sub-aggregate is open */
...
/* Emit the SDF aggregate header ("begin BVH") once, before the first
   light/surface/primitive.  No-op for other formats; lib_close writes
   the matching "end". */
void    lib_sdf_scene_begin PARAMS((void));
