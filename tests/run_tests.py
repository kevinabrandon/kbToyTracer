...
CASES = [
    ...
    # The next three scenes were generated by the revived SPD port
    # (tools/kbSPD, e.g. "teapot -s 4"): they pin the emitter's output format
    # end-to-end -- per-surface nested BVH groups (material lines must not
    # leak onto the previous surface's last primitive), point lights as
    # emissive points, and the derived vpdist framing.
    ("spd-teapot",             "tests/spd-teapot.sdf",         {},
     "The SPD teapot (tools/kbSPD: teapot -s 4): bezier patches tessellated "
     "to smooth_triangles over the checkerboard of quads, two point lights."),
    ("spd-sphereflake",        "tests/spd-balls.sdf",          {},
     "The SPD sphereflake (balls -s 2): 91 analytic spheres from the "
     "recursive generator, mirror-ish finish, on the one-quad beige floor."),
    ("spd-mount",              "tests/spd-mount.sdf",          {},
     "The SPD fractal mountain (mount -s 4): flat-shaded terrain triangles "
     "plus the four 90%-refractive crystal spheres -- pins dense glass and "
     "the deferred (additive) Phong highlights on refractive surfaces."),
]
